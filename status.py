...
Inspects which pilot artifacts exist for a year and prints the next action, so a
...
def step_status(batch):
    ...
    raw = [
        ("export.json",          P["export"],    f"pilot {batch}  (needs Photos access)"),
    ...
    if nxt is None:   # progressed to the end → final manual step (or re-apply if no snapshot)
        nxt = (f"pilot-tag {batch}  (needs Photos access) → delete the album with ⌘⌫"
